...
// Schema registry
import SCHEMAREG, { type SchemaKey } from './utils/schema-registry';
  ...
  const handleImportJson = useCallback(
    (data: any) => {
      ...
      if (!rawSchemaKey || typeof rawSchemaKey !== 'string') {
        message.error('Imported JSON is missing a valid "schema" key.');
        return;
      }

      if (!(rawSchemaKey in SCHEMAREG)) {
        message.error(`Unknown schema "${rawSchemaKey}" in imported JSON. Data not loaded.`);
        return;
      }
    ...
    [selectSchemaImmediate, setFormData]
  );
