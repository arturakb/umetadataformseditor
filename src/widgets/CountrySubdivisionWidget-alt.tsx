 ...
 * `root_geographicCoverage_geographicUnits_index_countryCode` and therefore
...
export default function CountrySubdivisionWidget(props: WidgetProps) {
   ...
   *
   * The widget’s `id` encodes its position in `geographicUnits`; this function uses
   * that index to read the corresponding `countryCode` from the latest form data.
   * For example, `id`=`root_geographicCoverage_geographicUnits_0_countryCode`.
   *
  ...
  const getCountryCode = () => {
    ...
    const countryCode = formData?.geographicCoverage?.geographicUnits?.[idx]?.countryCode ?? '';
    return countryCode;
  };
